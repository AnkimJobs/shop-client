...
export default {
  ...
  methods: {
    //1 生成二维码图片（请求回来的支付信息当中包含了一个生成二维码的数据并不是图片url（微信给我们的一个数据））
    //2 点击生成消息盒子
    //3 轮询
    //4 处理确认和取消的按钮  分别对应.then() .catch()
   async pay() {
      // With async/await
        ...
        try {
          ...
          // 设置循环定时器（轮询）,发送请求看用户对于这个订单是否支付成功
          //一般只开一个定时器
          if(!this.timer){
            this.timer=setInterval(async () => {
               ...
               if(result.code===200){
              //  if(result.code===205){
                //  如果成功 1.跳转 2.关闭定时器 (两步) 3.关闭消息盒子 4.存储支付成功状态（为了后期用户点击按钮的判断依据）
                this.payStatus= 1
                clearInterval(this.timer)
                this.timer=null
                this.$msgbox.close() 
