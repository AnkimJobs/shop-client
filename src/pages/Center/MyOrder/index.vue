    ...
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="order in myOrderList" :key="order.id">
          ...
          <tbody>
            <tr
              v-for="(deteil, index) in order.orderDetailList"
              :key="deteil.id"
            >
              <td width="60%">
                <div class="typographic">
                  <img src="./images/goods.png" />
                  <a href="#" class="block-text">{{ deteil.skuName }}</a>
                  <span>x{{ deteil.skuNum }}</span>
...
export default {
  ...
  methods: {
    ...
    async getOrderList(page = 1) {
      this.page = page;
      const result = await this.$API.reqOderList(this.page, this.limit);
      if (result.code === 200) {
